...
const RightSidebar = () => {
  ...
  const imageData = messages.filter((msg)=>(
     msg.image!==null
  ))
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll ${selectedUser ? "max-md:hidden" : ""}`}>
      <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light'>
        ...
        <h1 className='px-20 text-x1 font-medium mx-auto flex items-center gap-2'>
      ...
      <div className='px-5 text-xs'>
        ...
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {imageData.map((url, index) => (
            <div key={index} onClick={() => window.open(url.image)} className='cursor-pointer rounded'>
